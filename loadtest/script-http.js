...
export default function () {
  const url = 'http://localhost:8888/transcribe';
  const params = { headers: { Cookie: `JWT=${__ENV.JWT}`, 'Content-Type': 'application/json' } };
	...
	try {
      ...
      const normalizedActual = normalizeText(response.text);

      // Validate the transcription
      check(response, {
        [`Transcription matches expected (normalized) for request ${JSON.stringify(response)}`]: () =>
          //normalizedActual === normalizedExpected,
	  levenshteinDistance(normalizedActual, normalizedExpected) <= 10,
      });

	  // Sleep for the duration of the sample before sending the next request
	  console.warn('sleeping for ' + sample.duration);
	  sleep(sample.duration);
	  console.warn('slept for ' + sample.duration);
	} catch(e) {
